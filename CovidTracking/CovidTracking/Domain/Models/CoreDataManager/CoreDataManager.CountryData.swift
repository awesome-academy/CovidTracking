//
//  CoreDataManager.CountryData.swift
//  CovidTracking
//
...
extension CoreDataManager.FollowCountryManager {
    ...
    func fetchAllCountryData() -> Observable<[FollowCountry]> {
        return Observable.create { observable in
            do {
                ...
                observable.onNext(data ?? [FollowCountry]())
            } catch {
    ...
    func addNewCountry(countryName: String) -> Completable {
        return Completable.create { completable in
            let fetchRequest = NSFetchRequest<NSFetchRequestResult>(entityName: "FollowCountry")
            ...
            do {
                ...
                if check.isEmpty {
                    let _ = FollowCountry(context: CoreDataManager.shared.context).then {
                        $0.name = countryName
                    }
